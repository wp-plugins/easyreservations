...
<script>
	function easyreservations_build_datepicker(){
		jQuery("#datepicker, #datepicker2").datepicker( { dateFormat: 'dd.mm.yy' });
	}
</script>
<?php
...
if(isset($theMail) AND isset($theID)){
	...
	if(isset($_POST['thename']) AND empty($error)){ //When Check gives no error Insert into Database and send mail
		...
		echo '<div class="easy_form_success">'.__( 'Your Reservation was edited' , 'easyReservations' ).'</div>';
	...
	echo '<div style="background:#f9f9f9; padding:5px;margin:5px;text-align:center;"><b style="font-weight:bold;">'.__( 'Status' , 'easyReservations' ).': '.reservations_status_output($approve).' | '.__( 'Price' , 'easyReservations' ).': '.easyreservations_get_price($theID,1).' | '.__( 'Left' , 'easyReservations' ).': '.reservations_format_money($left, 1).'</b>'.$paypal.'</div>'; ?>

		<div id="showError" class="showError" style="margin-left:auto;margin-right:auto;"><?php echo $pastError; if(isset($error)) echo $error; ?></div>
		<form method="post" id="easyFrontendFormular" name="easyFrontendFormular" style="width:400px;margin-left:auto;margin-right:auto;">
		<input name="pricenonce" type="hidden" value="<?php echo wp_create_nonce('easy-price'); ?>">
			<input name="editID" type="hidden" value="<?php echo $theID;?>">
			<input name="old_email" type="hidden" value="<?php echo $editQuerry[0]->email;?>">
			<input name="easy-user-edit" type="hidden" value="<?php echo wp_create_nonce('easy-user-edit'); ?>">
			<label><?php echo __( 'Name' , 'easyReservations' );?><span class="small"><?php echo __( 'Your name' , 'easyReservations' );?></span></label><input type="text" name="thename" onchange="easyreservations_send_validate();" value="<?php echo $editQuerry[0]->name; ?>">
			<label><?php echo __( 'eMail' , 'easyReservations' );?><span class="small"><?php echo __( 'Your email' , 'easyReservations' );?></span></label><input type="text" name="email" onchange="easyreservations_send_price('front');easyreservations_send_validate();" value="<?php echo $editQuerry[0]->email; ?>">
			<label><?php echo __( 'From' , 'easyReservations' );?><span class="small"><?php echo __( 'The arrival date' , 'easyReservations' );?></span></label><input type="text" name="from" onchange="easyreservations_send_price('front');easyreservations_send_validate();" id="datepicker" value="<?php echo date("d.m.Y", strtotime($editQuerry[0]->arrivalDate)); ?>">
			<label><?php echo __( 'To' , 'easyReservations' );?><span class="small"><?php echo __( 'The departure date' , 'easyReservations' );?></span></label><input type="text" name="to" onchange="easyreservations_send_price('front');easyreservations_send_validate();" id="datepicker2" value="<?php  echo date("d.m.Y", strtotime($editQuerry[0]->arrivalDate)+(86400*$editQuerry[0]->nights));?>">
			<label><?php echo __( 'Persons' , 'easyReservations' );?><span class="small"><?php echo __( 'The amount of persons' , 'easyReservations' );?></span></label><select name="persons" onchange="easyreservations_send_price('front');easyreservations_send_validate();"><?php echo easyReservations_num_options(1,50,$persons); ?></select>
			<?php if(isset($childs) AND $childs != ""){ ?><label><?php echo __( 'Children\'s' , 'easyReservations' );?><span class="small"><?php echo __( 'The amount of children\'s' , 'easyReservations' );?></span></label><select name="childs" onchange="easyreservations_send_price('front');"><?php echo easyReservations_num_options(0,50,$childs); ?></select><?php } ?>
			...
			<label><?php echo __( 'Room' , 'easyReservations' );?><span class="small"><?php echo __( 'Choose the room' , 'easyReservations' );?></span></label><select  name="room" id="room" onChange="<?php if($isCalendar){ ?>document.CalendarFormular.room.value=this.value;easyreservations_send_calendar('shortcode');<?php } ?>easyreservations_send_price('front');easyreservations_send_validate();"><?php echo reservations_get_room_options($editQuerry[0]->room); ?></select>
			<label><?php echo __( 'Offer' , 'easyReservations' );?><span class="small"><?php echo __( 'Choose the offer' , 'easyReservations' );?></span></label><select  name="offer" id="offer" onchange="easyreservations_send_price('front');easyreservations_send_validate();"><?php echo __($specials);?><option  value="0"><?php echo __( 'None' , 'easyReservations' );?></option><?php echo reservations_get_offer_options($special); ?></select>
			<?php
			...
			if(!empty($editQuerry[0]->customp)){
				...
				foreach($customsmerges as $thenumber2 => $customprice){
					...
					?><label><?php echo __($custompriceexp[0]); ?><span class="small"><?php echo __( "Pay service" , "easyReservations" ); ?></span></label><span class="formblock"><input type="hidden" name="customp_value_<?php echo $thenumber2; ?>" value="<?php echo $customprice; ?>"><b><?php echo $pricexplode[0]; ?>:</b> <?php echo $pricexplode[1].' &'.get_option("reservations_currency"); ?>;<input type="checkbox" name="custom_price<?php echo $thenumber2; ?>" id="custom_price<?php echo $thenumber2; ?>" value="test:<?php echo $pricexplode[1]; ?>" onchange="easyreservations_send_price('front');" checked ></span><?php
				}
			} ?>
			...
			<div style="text-align:center">
				<?php if($resPast == 0){ ?><input type="submit" onclick="document.getElementById('easyFrontendFormular').submit(); return false;" value="<?php printf ( __( 'Submit' , 'easyReservations' ));?>"><?php } ?>
				<span class="showPrice" style="margin-left:10px"><?php echo __( 'Price' , 'easyReservations' ); ?>: <span id="showPrice" style="font-weight:bold;"><b>0,00</b></span> &<?php echo get_option("reservations_currency"); ?>;</span>
			</div>
		</form>
		<?php
		...
		<form method="post" id="easyFrontendFormular" style="padding-left:106px">
			<input type="hidden" value="<?php echo $prefix; ?>" name="captcha_prefix">
			<input name="easy-user-edit-login" type="hidden" value="<?php echo wp_create_nonce('easy-user-edit-login'); ?>">
			...
			<label><?php echo __( 'Captcha' , 'easyReservations' );?><span class="small"><?php echo __( 'Type in code' , 'easyReservations' );?></span></label><input type="text" name="captcha_value" style="width:40px;"><img style="vertical-align:middle;" src="<?php echo RESERVATIONS_LIB_DIR.'/captcha/tmp/'.$url; ?>">
