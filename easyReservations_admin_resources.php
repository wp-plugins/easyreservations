...
?><div id="icon-themes" class="icon32"></div><h2 style="font-family: Arial,sans-serif; font-weight: normal; font-size: 18px;">Resources<div id="wrap"><br>
<?php
...
foreach($categoryids as $categoryid){
	 if($categoryid == $room_category){ $roomoroffer='Rooms'; $roo = 'room'; } else { $roomoroffer='Offers'; $roo = 'offer'; }
?>	<h2 style="font-family: Arial,sans-serif; font-weight: normal; font-size: 20px; margin:9px 0 2px 0"><?php echo $roomoroffer; ?><a class="add-new-hari" href="admin.php?page=reservation-resources&addresource=<?php echo $roo; ?>" rel="simple_overlay" style="width:20px;heigth:29px;"href="#">Add new <?php echo substr($roomoroffer, 0, -1); ?></a></h2>
			...
			foreach($filterouts as $filterout){ //foreach filter array
				...
				if(reservations_is_room($allroom->ID)){ $countallrooms = mysql_num_rows(mysql_query("SELECT id FROM ".$wpdb->prefix ."reservations WHERE approve='yes' AND room='$allroom->ID'")); } // number of total rows in the database				
					...
					?><tr class="<?php echo $class; ?>">
							<td style="text-align:center; vertical-align:middle;"><a href="post.php?post=<?php echo $allroom->ID; ?>&action=edit" title="<?php echo __( 'edit' , 'easyReservations' ); ?>"><?php if(function_exists('get_the_post_thumbnail')) echo get_the_post_thumbnail($allroom->ID, array(70,70)); ?></a></td>
...

		...
		if($action=='set_groundprice'){
			if($roomoroffer == "room"){
				...
				$replacegprice=str_replace(",", ".", $gpricepost);
				echo $replacegprice;
		...
		foreach( $roomcategories as $roomcategorie ){
			...
			$roomjsarray .= ' <b> '.$roomcategorie->post_title.'</b>:<input type="hidden" name="idroom'.$counroooms.'" id="idroom'.$counroooms.'" value="'.$roomcategorie->ID.'"><input type="Text" id="priceroom'.$counroooms.'" name="priceroom'.$counroooms.'" value="Price" style="width:55px">';
			...
			foreach($explgpprices as $explgpprice){
				...
				if($explidgpprice[0] == $roomcategorie->ID){
					$roomgpadd .= ' '.$roomcategorie->post_title.': <input type="hidden" name="idgproom'.$counroooms.'" id="idgproom'.$counroooms.'" value="'.$roomcategorie->ID.'"><input type="Text" id="gppriceroom'.$counroooms.'" name="gppriceroom'.$counroooms.'" value="'.$explidgpprice[1].'" style="width:60px"> ';
					$wassetted++;
					if($counroooms > 2){
						$roomgpadd .= '<br>';
					}
			...
			if( $wassetted == 0) $roomgpadd .= $roomcategorie->post_title.': <input type="hidden" name="idgproom'.$counroooms.'" id="idgproom'.$counroooms.'" value="'.$roomcategorie->ID.'"><input type="Text" id="gppriceroom'.$counroooms.'" name="gppriceroom'.$counroooms.'" value="Price" style="width:60px"><br>';
		...
		if($roomoroffer == "room"){  
			$groundpricefield='<b>'.__( 'Groundprice' , 'easyReservations' ).':</b> <input type="text" value="'.get_post_meta($resourceID, 'reservations_groundprice', true).'" style="width:60px" name="groundprice"> &'.get_option("reservations_currency").';';
			...
			<tr>
				<td valign="top" style="width:64%">
					<table class="widefat">
							...
							<tbody>
								<tr>
									<td style="width:70px;" valign="top"><?php if(function_exists('get_the_post_thumbnail')) echo get_the_post_thumbnail($resourceID, array(70,70)); ?></td>
						...
						<tbody>
							<tr>
								<td>
									...
									<?php if($roomoroffer == "room"){ ?><input type="hidden" name="action2" id="action2" value="set_roomcount"><b><?php printf ( __( 'Roomcount' , 'easyReservations' ));?>:</b> <input type="text" name="roomcount" style="width:30px;" value="<?php echo $reservations_current_room_count; ?>" style="margin: 2px;"><?php } ?>
...
<?php if($roomoroffer == "offer"){ ?>
					<table class="widefat" style="margin-top:7px">
						<thead>
							<tr>
								<th><?php printf ( __( 'Offer Box Informations' , 'easyReservations' ));?></th>
							</tr>
						</thead>
						<tbody>
							<tr>
								<td>
									...
									From - To: <input type="text" name="fromtobox" value="<?php echo $reservations_current_value_fromto;?>"><br>
									Percent/Price: <input type="text" name="pricebox" value="<?php echo $reservations_current_value_percent;?>" style="margin: 2px;"><br>
									Short Description: <input type="text" name="descbox" value="<?php echo $reservations_current_value_short;?>" style="margin: 2px;"><br>
									<a style="float:right" href="javascript:{}" onclick="document.getElementById('set_infobox').submit(); return false;" class="button-secondary"><span><?php printf ( __( 'Set' , 'easyReservations' ));?></span>
									</form>
								</td>
							</tr>
						</tbody>
...
<?php	
		...
		echo $prompt; ?><form method="post" action="" name="addresource" id="addresource">
	<input type="hidden" name="roomoroffer" value="<?php echo $addresource; ?>">
		<table class="widefat" style="width:340px;">
			<thead>
				<tr>
		...
		<a href="javascript:{}" onclick="document.getElementById('addresource').submit(); return false;" class="button-primary"><span><?php printf ( __( 'Add' , 'easyReservations' )); echo ' '.$roomoroffer; ?></span></a>
	</form>
	<script>
