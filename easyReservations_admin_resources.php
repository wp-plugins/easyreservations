							...
							foreach($theFilters as $nummer => $filter){ //foreach filter array
								if($filter['type'] == 'price') {
									...
									<tr class="<?php echo $class; ?>">
										...
										<td class="resourceType" style="background:#BF4848;border-left:0px;width:60px;cursor:pointer">P</td>
							...
							if($countfilter > 0){
								foreach($theFilters as $nummer => $filter){ //foreach filter array
									...
									if($filter['type']=="stay"){
										...
										$bgcolor='#1CA0E1';
										$condition_string = sprintf(__('If guest reservates %s days before his arrival he\'ll get an discount of','easyReservations'), '<b>'.$filter['cond'].'</b>');
										...
										<div id="filter_form_time_cond" class="hide-it">
											...
											<span style="min-width:99%;display:block;float:left">
												...
												<div style="margin:3px;width:90px;float:left;">
													...
													<span style="margin:3px;"><input type="checkbox" name="price_filter_unit_days[]" value="6"> <?php echo __( 'Suturday' , 'easyReservations' ); ?></span><br>
