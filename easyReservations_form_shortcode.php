...
function reservations_form_shortcode($atts){
	...
	<link href="<?php echo WP_PLUGIN_URL;?>/easyreservations/css/forms/form_<?php echo get_option("reservations_style_form"); ?>.css" rel="stylesheet" type="text/css"/>
	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
			var dates = $( "#easy-form-from, #easy-form-to" ).datepicker({
				dateFormat: 'dd.mm.yy',
				minDate: -1,
	...
	if(isset($_POST['action'])) { // Check and Set the Form Inputs
		...
		if(is_numeric(strtotime($_POST['from'])) AND strtotime($_POST['from']) > time()-86400) {
		...
		if(isset($_POST['to'])){
			if(!empty($_POST['to']) AND strtotime($_POST['to'])-time() > '0' AND (ereg ("([0-9]{1,2})-([0-9]{1,2})-([0-9]{4})", $_POST['to']) OR ereg ("([0-9]{1,2})/([0-9]{1,2})/([0-9]{4})", $_POST['to']) OR ereg ("([0-9]{1,2}).([0-9]{1,2}).([0-9]{4})", $_POST['to']) OR ereg ("([0-9]{4}).([0-9]{1,2}).([0-9]{1,2})", $_POST['to']) OR ereg ("([0-9]{4})-([0-9]{1,2})-([0-9]{1,2})", $_POST['to']) OR ereg ("([0-9]{4})/([0-9]{1,2})/([0-9]{1,2})", $_POST['to']))) {
		...
		if(!empty($_POST['email']) AND strlen($_POST['email']) < 40 AND strlen($_POST['email']) > 5 AND eregi($pattern_mail, $_POST['email'])){
	...
	if(isset($_POST['action']) AND !$error) { //When Check gives no error Insert into Database and send mail
		...
		echo '<b>'.__( 'Your reservation was sent' , 'easyReservations' ).'</b><br><br><br>';
	...
	foreach($edgetworemoves as $fields){
		...
		if($field[0]=="date-from"){
			...
			$theForm=preg_replace('/\['.$fields.'\]/', '<select name="nights">'.$nights_options.'</select>', $theForm);
			...
			if($field[1]=="Select"){
				...
				$theForm=preg_replace('/\['.$fields.'\]/', '<select name="persons" onchange="'.$price_action.'">'.$person_options.'</select>', $theForm);
			} elseif($field[1]=="text"){
			...
			if($field[1]=="Select"){
				if(isset($field[2])) $number=$field[2]; else $number = 6;
				$childs_options='';
				for($count = 0; $count <= $number; $count++){  $childs_options.='<option value="'.$count.'">'.$count.'</option>'; }
				$theForm=preg_replace('/\['.$fields.'\]/', '<select name="childs" onchange="'.$price_action.'">'.$childs_options.'</select>', $theForm);
			} elseif($field[1]=="text"){
				$theForm=preg_replace('/\['.$fields.'\]/', '<input name="childs" type="text" size="70px" onchange="'.$price_action.'">', $theForm);
			}
		} elseif($field[0]=="thename"){
			$theForm=preg_replace('/\['.$fields.'\]/', '<input type="text" name="thename" value="'.$name_form.'" onchange="'.$validate_action.'">', $theForm);
			...
			$theForm=str_replace('['.$fields.']', '<select name="country">'.easyReservations_country_select('').'</select>', $theForm);
			...
			$theForm=preg_replace('/\['.$fields.'\]/', '<input type="text" name="captcha_value" style="width:40px;"><img style="vertical-align:middle;" src="'.RESERVATIONS_LIB_DIR.'/captcha/tmp/'.$url.'"><input type="hidden" value="'.$prefix.'" name="captcha_prefix">', $theForm);
			...
			if($isCalendar == true) $calendar_action = 'document.formular.room.value=this.value;easyRes_sendReq_Calendar();'; else $calendar_action = '';
			$theForm=str_replace('['.$fields.']', '<select name="room" id="form_room" onChange="'.$calendar_action.$price_action.$validate_action.'">'.reservations_get_room_options().'</select>', $theForm);
			...
			if($field[1]=="text"){
				...
				if(preg_match("/^[0-9]+$/", $valuefield)){
					...
					$theForm=preg_replace('/\['.$fields.'\]/', '<select name="'.$field[2].'">'.$custom_select.'</select>', $theForm);
				} elseif(preg_match("/^[a-zA-Z0-9_]+$/", $valuefield)){
			...
			$valuefield=str_replace('"', '', $field[3]);
			$priceFieldNames.=$field[2].',';
			if($field[1]=="checkbox"){
				$explodeprice=explode(":", $valuefield);
				$theForm=preg_replace('/\['.$fields.'\]/', '<input id="custom_price'.$customPrices.'" type="checkbox" name="'.$field[2].'" value="'.$explodeprice[0].':'.$explodeprice[1].'">', $theForm);
			} elseif($field[1]=="radio"){
				if(preg_match("/^[a-zA-Z0-9_]+$/", $valuefield)){
					$explodeprice=explode(":", $valuefield);
					$theForm=preg_replace('/\['.$fields.'\]/', '<input id="custom_price'.$customPrices.'" type="radio" name="'.$field[2].'" value="'.$explodeprice[0].':'.$explodeprice[1].'"> '.$explodeprice[0].': '.reservations_format_money($explodeprice[1]).' &'.get_option("reservations_currency").';', $theForm);
				} elseif(strstr($valuefield,",")) {
					$valueexplodes=explode(",", $valuefield);
					$custom_radio = '<pre>';
					foreach($valueexplodes as $value){
						$explodeprice=explode(":", $value);
						if($value != '') $custom_radio .= '<input id="custom_price'.$customPrices.'" type="radio" name="'.$field[2].'" value="'.$explodeprice[0].':'.$explodeprice[1].'"> '.$explodeprice[0].': '.reservations_format_money($explodeprice[1]).' &'.get_option("reservations_currency").';<br>';
					}
				...
				if(preg_match("/^[a-zA-Z0-9_]+$/", $valuefield)){
					$explodeprice=explode(":", $valuefield);
					$theForm=preg_replace('/\['.$fields.'\]/', '<select id="custom_price'.$customPrices.'" name="'.$field[2].'"><option value="'.$explodeprice[0].':'.$explodeprice[1].'">'.$explodeprice[0].': '.reservations_format_money($explodeprice[1]).' &'.get_option("reservations_currency").';</option></select>', $theForm);
			...
			if($field[1]=="select"){
				if($isCalendar == true) $calendar_action = 'document.formular.offer.value=this.value;easyRes_sendReq_Calendar();'; else $calendar_action = '';
				$theForm=preg_replace('/\['.$fields.'\]/', '<select name="specialoffer" id="form_offer" onchange="'.$calendar_action.$price_action.$validate_action.'"><option value="0" select="selected">'. __( 'None' , 'easyReservations' ).'</option>'.reservations_get_offer_options().'</select>', $theForm);
			} elseif($field[1]=="box"){
				...
				if($cate==$special_offer_cat){
					...
					$special_offer_promt.='<img src="'.$image_url.'" style="height:55px; width:55px; border:0px; margin:0px 10px 0px 0px; padding:0px;" class="alignleft"> '.__( 'You\'ve choosen' , 'easyReservations' ).': <b>'.__(get_the_title($theIDs)).'</b><img style="float: right;" src="'.RESERVATIONS_IMAGES_DIR.'/close.png" onClick="'."removeElement('parent','child')".';'.$price_action.'"><br>'.__( 'Available' , 'easyReservations' ).': '.__($fromto[0]).'<br>'.__($desc).' <input type="hidden"  name="specialoffer" value="'.$theIDs.'" /></div>';
