			...
			$fromto = get_post_meta($theIDs, 'fromto');
				...
				$results = $wpdb->get_results($posts);
					...
					if($cate==$special_offer_cat){
						...
						$special_offer_promt.='<img src="'.$image_url.'" style="height:55px; width:55px; border:0px; margin:0px 10px 0px 0px; padding:0px;" class="alignleft"> '.__( 'You\'ve choosen' , 'easyReservations' ).': <b>'.__(get_the_title($theIDs)).'</b><img style="float: right;" src="'.WP_PLUGIN_URL.'/easyreservations/images/close.png" onClick="'."removeElement('parent','child')".'"><br>'.__( 'Available' , 'easyReservations' ).': '.__($fromto[0]).'<br>'.__($desc).' <input type="hidden"  name="specialoffer" value="'.$theIDs.'" /></div>';
