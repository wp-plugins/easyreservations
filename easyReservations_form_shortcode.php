...
.warning {
	...
	width:250px;
			...
			if($_POST['action'] AND !$error) { //When Check gives no error Insert into Database and send mail
				...
				$wpdb->query( $wpdb->prepare("INSERT INTO ".$wpdb->prefix ."reservations(name,  email, notes, nights, arrivalDate, dat, room, number, special, custom, customp ) 
				VALUES ('$name_form', '$email_form', '$message_form', '$nights_form', '$arrivaldate_form', '$month_form', '$room_form', '$persons_form', '$specialoffer_form', '$custom_form', '$custom_price' )" ) );

				echo '<b>'.__( 'Your Reservation was sent' , 'easyReservations' ).'</b><br>';
				...
				$edgetworemovess=str_replace(']', '', $edgeoneremoave);
					foreach($edgetworemovess as $fieldsx){
						...
						elseif($field[0]=="arrivaldate"){
							$emailformation=preg_replace('/'.$fieldsx.'/', date("d.m.Y", strtotime($arrivaldate_form)), $emailformation);
						}
						elseif($field[0]=="departuredate"){
							$emailformation=preg_replace('/'.$fieldsx.'/', ''.date("d.m.Y", strtotime($_POST['to'])).'', $emailformation);
						}
			...
			$cate=$cates[0]->term_id;
						...
						$room=reservations_get_room_options();
						 $special_option=reservations_get_offer_options();
