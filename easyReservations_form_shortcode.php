			...
			$fromto = get_post_meta($theIDs, 'fromto');
					...
					if($special_offer_style=="1") {
								...
								if($cate==$special_offer_cat){ 
										...
										$special_offer_promt.=get_the_post_thumbnail($theIDs,array(55,55), array('class' => 'alignleft')).' '.__( 'You\'ve choosen:' , 'easyReservations' ).' <b>'.__($b).'</b><img style="float: right;" src="'.WP_PLUGIN_URL.'/easyReservations/images/close.png" onClick="'."removeElement('parent','child')".'"><br>'.__( 'Available:' , 'easyReservations' ).' '.__($fromto[0]).'<br>'.__($desc).' <input type="hidden"  name="specialoffer" value="'.__($theIDs).'" /></div>';
