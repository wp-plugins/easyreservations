...
<link href="<?php echo WP_PLUGIN_URL;?>/easyreservations/css/forms/form_<?php echo $style; ?>.css" rel="stylesheet" type="text/css"/>
	...
	if(isset($_POST['easy-user-add']) && empty($error) && isset($from)) { //When Check gives no error Insert into Database and send mail
	...
	foreach($edgetworemoves as $fields){
		...
		if($field[0]=="date-from"){
			...
			if($field[1]=="room"){
				...
				$theForm=preg_replace('/\['.$fields.'\]/', '<input type="hidden" name="offer" value="'.$field[2].'">', $theForm);
			} elseif($field[1]=="from"){
	...
	$finalform.='<div class="easyFrontendFormular"><form method="post" id="easyFrontendFormular" name="easyFrontendFormular" class=""><input name="easy-user-add" type="hidden" value="'.wp_create_nonce('easy-user-add').'">'.$finalformedgesremoved.'<!-- Provided by easyReservations free Wordpress Plugin http://www.feryaz.de --></form></div>';
