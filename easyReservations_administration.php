						...
						$rowcount="0";
							...
							while($roomcounty > $rowcount){
								...
								while($showdatenumber_start < $showdatenumber_end){
									...
									$sql_R2 = "SELECT name, nights, arrivalDate, room FROM ".$wpdb->prefix ."reservations WHERE room='$roomsidentify' AND roomnumber='$rowcount' AND approve='yes' AND  roomnumber != '' AND TIMESTAMP(arrivalDate) + INTERVAL 14 DAY > $datumtoday AND arrivalDate < $dateplus14 ORDER BY arrivalDate ASC";
												...
												<table  class="widefat" style="width:99%;"> <!-- Main Table //-->
													...
													<?php
														...
														$result = mysql_query($sql) or die ('Error, query failed');
															...
															while ($row = mysql_fetch_assoc($result)) {
																	...
																	$timestampend=$anznights+$timpstampanf;
																	 ...
																	 if($special=="0") { // Calculate Price for Rooms for each day 
																		$xsr="0";
																		$roomprice=explode("-", get_post_meta($room, 'price', true)); 
																		$price=0;
																		while($nights > $xsr) {
																		$xsr++;
																		$plusanznights=60*60*24*$xsr;
																		$dateofday=$timpstampanf+$plusanznights;
																		if($dateofday >= strtotime($season1[0]) AND $dateofday <= strtotime($season1[1])) {$price+=$roomprice[0]; }
																		if($dateofday >= strtotime($season2[0]) AND $dateofday <= strtotime($season2[1])) {$price+=$roomprice[1]; }
																		if($dateofday >= strtotime($season3[0]) AND $dateofday <= strtotime($season3[1])) {$price+=$roomprice[2]; }
																		if($dateofday >= strtotime($season4[0]) AND $dateofday <= strtotime($season4[1])) {$price+=$roomprice[3]; }
																		if($dateofday >= strtotime($season5[0]) AND $dateofday <= strtotime($season5[1])) {$price+=$roomprice[4]; }
																		}
																	} else { // Calculate Price for Special Offer 

																		$specialprices=explode("-", get_post_meta($special, 'price', true)); 
																			foreach($specialprices as $specialprice) {
																				$specialp=explode(":", $specialprice);
																					if($room==$specialp[0]) $price = $specialp[1] * $nights;
																				}

																	}
																	if(get_option('reservations_price_per_persons') == '1') { $price=$price*$person; } // Calculate Price when choosing "Calculate per person"
																	$nr++; 
														...
														<tr class="<?php echo $class; ?> test" height="47px"><!-- Main Table Body //-->
															...
															<td width="7%" nowrap><?php if($price==0) echo "Out of Season<br>Please set Season"; else echo $price."&".get_option('reservations_currency').";" ; ?><?php $csv_output .= $price . ", "; ?></td>
			...
			<table class="widefat" style="width:100%;">
				...
				<tbody>
					...
					<table class="widefat">
						...
						<tbody>
								<tr valign="top">
									<td style="vertical-align:middle;"><?php printf ( __( 'Website for Plugin is online!' , 'easyReservations' ));?><br><?php printf ( __( 'With a Documentation, a Vote-System for future Features and a Contact Page' , 'easyReservations' ));?><br><b><a href="http://www.feryaz.de/easyreservations/">feryaz.de/easyReservations</a></td>
								</tr>
						</tbody>
					</table>
		</td></tr></table><br>
