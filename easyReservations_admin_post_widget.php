...
function reservations_save_postdata( $post_id ) {
 ...
 $reservations_current_value_fromto = get_post_meta($post->ID, 'reservations_fromto', TRUE);
 $reservations_current_value_percent = get_post_meta($post->ID, 'reservations_percent', TRUE);
 $reservations_current_value_short = get_post_meta($post->ID, 'reservations_short', TRUE);
 $reservations_current_value_price = get_post_meta($post->ID, 'reservations_groundprice', TRUE);
 $reservations_current_room_count = get_post_meta($post->ID, 'roomcount', TRUE);
	...
	if($reservations_current_value_fromto) {
		if($reservations_from_to == "") delete_post_meta($post->ID,'reservations_fromto');
		else update_post_meta($post->ID,'reservations_fromto',$reservations_from_to);
	}
	elseif($reservations_from_to != ""){
		 add_post_meta($post->ID,'reservations_fromto',$reservations_from_to,TRUE);
	}
	
	my_meta_clean($reservations_percent);
	if ($reservations_current_value_percent) {
		if($reservations_percent == "")  delete_post_meta($post->ID,'reservations_percent');
		else update_post_meta($post->ID,'reservations_percent',$reservations_percent);
	}
	elseif($reservations_percent != "") {
		add_post_meta($post->ID,'reservations_percent',$reservations_percent,TRUE);
	}

	my_meta_clean($reservations_short);
	if ($reservations_current_value_short) {
		if($reservations_short == "") delete_post_meta($post->ID,'reservations_short');
		else update_post_meta($post->ID,'reservations_short',$reservations_short);
	}
	elseif($reservations_short != "") {
		add_post_meta($post->ID,'reservations_short',$reservations_short,TRUE);
	}
