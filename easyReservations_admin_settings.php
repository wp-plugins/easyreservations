...
function reservation_settings_page() { //Set Settings
		...
		if(isset($action) AND $action == "er_main_set"){
			...
			$prompt = '<div class="updated"><p>'.__( 'Main settings saved' , 'easyReservations' ).'</p></div>';
		}
		...
		if(isset($action) AND $action == "reservations_email_settings"){//Set Reservation Mails
			...
			$reservations_email_to_user_subj = $_POST["reservations_email_to_user_subj"];
			update_option("reservations_email_to_user_msg",$reservations_email_to_user_msg);
			update_option("reservations_email_to_user_subj",$reservations_email_to_user_subj);
		...
		if(isset($action) AND $action == "reservation_change_permissions"){ // Change a form
			if(is_admin()){
				...
				$prompt = '<div class="updated"><p>'.__( 'Permission changed' , 'easyReservations' ).'</p></div>';
			} else $prompt = '<div class="error"><p>'.__( 'Only admins can change the permission for easyReservations' , 'easyReservations' ).'</p></div>';
		}
				...
				<input type="hidden" name="action" value="reservation_change_permissions" id="reservation_change_permissions">
					<table class="<?php echo RESERVATIONS_STYLE; ?>" style="width:100%;margin-top:7px;" cellspacing="0" cellpadding="0">
						...
						<tbody>
								...
								$permissions = array( 1 => array(__( 'Contributor' , 'easyReservations' ), 'edit_posts'), 2 => array(__( 'Author' , 'easyReservations' ), 'publish_posts'), 3 => array(__( 'Editor' , 'easyReservations' ), 'manage_categories'), 4 => array(__( 'Administrator' , 'easyReservations' ), 'manage_options'), 5 => array(__( 'Super Admin' , 'easyReservations' ), 'manage_network') ); 
			...
			<tbody>
				...
				<tr valign="top">
					...
					<form id="form1" name="form1">
						...
						&nbsp;<a href="javascript:resetform();" class="easySubmitButton-primary" style="line-height:1;"><?php printf ( __( 'Reset' , 'easyReservations' ));?></a>
					</form>
					<form method="post" action="admin.php?page=reservation-settings&site=form<?php if($formnameget!=""){ echo '&form='.$formnameget; } ?>"  id="reservations_form_settings" name="reservations_form_settings">
...
function jumpto(x){ // Chained inputs;
	...
	if(thetext1 == false){
		if (x == "custom") {
			...
			thetext1 = true;
			document.form1.jumpmenu.disabled=true;
			var Output  = '&nbsp;<a href="javascript:AddOne()" class="easySubmitButton-primary" style="line-height:1;"><b>Add</b></a>';
				if(x == "error"){
			...
			document.form1.jumpmenu.disabled=true;
			var Output  = '&nbsp;<a href="javascript:AddTwo()" class="easySubmitButton-primary" style="line-height:1;"><b>Add</b></a>';
			document.getElementById("Text4").innerHTML += Output;
		...
		if (x == "textarea" || x == "text" || x == "check"){
			...
			Help += '<br> &emsp; <i><span style="color:#FF0000">select Name</span>:<span style="color:#16A039">10</span> // <?php echo __( 'if guest checks the checkbox the price will increase by' , 'easyReservations' ); echo ' '.reservations_format_money(10, 1); ?></i>'
			Help += '<br> &emsp; <i><span style="color:#FF0000">want Breakfast</span>:<span style="color:#16A039">25.24</span> // <?php echo __( 'if guest checks the checkbox the price will increase by' , 'easyReservations' ); echo ' '.reservations_format_money(25.24, 1); ?></i>';
			Help += '<br> &emsp; <i><span style="color:#FF0000">no Laundry</span>:<span style="color:#16A039">-30.36</span> // <?php echo __( 'if guest checks the checkbox the price will decrease by' , 'easyReservations' ); echo ' '.reservations_format_money(-30.36, 1); ?></i>';
			...
			if(first == "price"){
				...
				Help += '<br> &emsp; <i><span style="color:#FF0000">select Name</span>:<span style="color:#16A039">10</span> <?php echo __( 'if guest selects "selectname" the price will increase by' , 'easyReservations' ); echo ' '.reservations_format_money(10, 1); ?></i>'
				Help += '<br> &emsp; <i><span style="color:#FF0000">no Breakfast</span>:<span style="color:#16A039">-30.36</span>,<span style="color:#FF0000">wantBreakfast</span>:<span style="color:#16A039">25.24</span> <?php echo __( 'if Guest selects "noBreakfast" the price will decrease by' , 'easyReservations' ); echo ' '.reservations_format_money(-30.36, 1); ?></i>';
			...
			if(document.getElementById("jumpmenu").value=="persons"){
				...
				var Help = '<div class="explainbox"><b>1. <?php echo __( 'Select minimum and maximum number of persons to select' , 'easyReservations' ); ?></b>';
				Help += '<br><b>2. <?php echo __( 'Click on "Add" to add the persons field as select to the form' , 'easyReservations' ); ?></b></div><br>';
				document.getElementById("Helper").innerHTML = Help;
				
			} else {

				var Help = '<div class="explainbox"><b>1. <?php echo __( 'Select minimum and maximum number of childrens to select' , 'easyReservations' ); ?></b>';
				Help += '<br><b>2. <?php echo __( 'Click on "Add" to add the childrens field as select to the form' , 'easyReservations' ); ?></b></div><br>';
				document.getElementById("Helper").innerHTML = Help;
	...
	if (end == 3) {
		var Output  = '&nbsp;<a href="javascript:AddThree()" class="easySubmitButton-primary" style="line-height:1;"><b>Add</b></a>';
		document.getElementById("Text4").innerHTML += Output;
	}
	if (end == 4) {
		var Output  = '&nbsp;<a href="javascript:AddFour()" class="easySubmitButton-primary" style="line-height:1;"><b>Add</b></a>';
		document.getElementById("Text4").innerHTML += Output;
	}
	if (end == 5) {
		var Output  = '&nbsp;<a href="javascript:AddFours()" class="easySubmitButton-primary" style="line-height:1;"><b>Add</b></a>';
		document.getElementById("Text4").innerHTML += Output;
	}
}
</script>
