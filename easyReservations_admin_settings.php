...
function reservation_settings_page() { //Set Settings
		...
		if(isset($action) AND $action == "reservation_settingss"){
			...
			$overview_size= $_POST["overview_size"];
			update_option("reservations_overview_size",$overview_size);
		...
		<input type="hidden" name="action" value="reservation_settingss">
			...
			<tbody>
				...
				<tr valign="top" class="alternate">
					...
					<td><select data-hex="true" title="<?php printf ( __( 'Style of the Reservations in the Reservation Overview' , 'easyReservations' ));?>" name="backgroundiffull" class="regular-text"><option select="selected" value="<?php echo $backgroundiffull;?>" ><?php echo $backgroundiffull;?></option><option value="green" >green</option><option value="red" >red</option><option value="yellow" >yellow</option><option value="blue" >blue</option><option value="pink" >pink</option></select>
					<select name="overview_size"><?php $elements = array( 'big', 'small'); 
					foreach($elements as $element){
						if($overview_size == $element) $string = 'selected'; else $string='';
						echo '<option value="'.$element.'" '.$string.'>'.$element.'</option>';
					}
					?></select>
					</td>
				</tr>
...
function jumpto(x){ // Chained inputs;
	...
	if(thetext1 == false){
		if (x == "custom") {
			...
			Help += '<br> &emsp; <i><b>Persons</b> <?php echo __( 'Fix a Departure Date to the Form; dont use it with [persons] in the same Form' , 'easyReservations' ); ?></i></div><br>';
			document.getElementById("Helper").innerHTML = Help;
...
Name: [thename] <br>eMail: [email] <br>From: [arrivaldate] <br>To: [departuredate] <br>Persons: [persons] <br>Room: [rooms] <br>Offer: [offers] <br>Message: [message]<br>[customs]";
	$emailstandart2="Your Reservation on Blogname has been approved.<br> 
[adminmessage]<br><br>
Reservation Details:<br>
Name: [thename] <br>eMail: [email] <br>From: [arrivaldate] <br>To: [departuredate] <br>Persons: [persons] <br>Offer: [offers] <br>Message: [message]<br>Price: [price]<br>[customs]";
