...
function reservations_post_widget() {
 ...
 $reservations_filter = get_post_meta($post->ID, 'reservations_filter', TRUE);
  ?><script src="<?php echo WP_PLUGIN_URL . '/easyreservations/js/filterhelp.js'; ?>"></script><small><?php printf ( __( 'Only fill this for Special Offers and Rooms' , 'easyReservations' ));?>; <a href=<?php echo '"http://www.feryaz.de"'; ?>><?php printf ( __( 'Help' , 'easyReservations' ));?></a></small><br><br>
...
<span id="Text"></span>
<select id="FirstNo"><option value="price">Price</option><option value="stay">Stay</option><option value="loyal">Loyal</option><option value="avail">Avail</option></select><input onfocus="if (this.value == 'Condition') this.value = '';"  style="border-color:#FF9393" type="text" id="SecondNo" name="cond" size="19" value="Condition" /><input  style="border-color:#AAFFC5" onfocus="if (this.value == 'Value') this.value = '';" type="text" id="ThirdNo" name="value" size="2" value="Value" /><input type="button" style="background-image: url(<?php echo WP_PLUGIN_URL; ?>/easyreservations/images/add.png); height:16px; width:16px;border:0px; vertical-align:middle;" onclick="Add()"/><br>
