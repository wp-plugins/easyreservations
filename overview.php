...
if(isset($_POST['more'])) $moreget = $_POST['more'];
...
<tr class="ov_resource_row" style="background:#EAE8E8">
	...
	while($roomcounty > $rowcount){  /* - + - FOREACH EXACTLY ROOM - + - */
		...
		if(isset($reservations[$rowcount])){
			foreach($reservations[$rowcount] as $reservation){
				...
				if(date($date_pat, $res_departure_stamp) == date($date_pat, $res_adate_stamp)){
					$round = round((strtotime(date("d.m.Y",$res_adate_stamp))+($interval/2)-$timesx)/$interval);
		...
		while($showdatenumber_start < $showdatenumber_end){
			...
			if($dateToday < time()) $background2="url(".RESERVATIONS_URL ."images/patbg.png?cond=".time().") repeat";
			else $background2='';
			...
			if($bypers){
				...
				if($daycount > 0){
					...
					$percent = 100/$roomcounty*$daycount;
					if($percent > 95) $farbe2 = '#cc3433';
					elseif($percent > 70) $farbe2 = '#ff7b00';
					elseif($percent > 35) $farbe2 = '#128d18';
					else $farbe2 = '#8FD996'; ?>
        <td id="<?php echo $roomID.'-'.$rowcount.'-'.$preparedCellcount; ?>"  title="<?php echo $title; ?>" <?php if($borderside == 0) echo 'class="er_overview_cell"'; ?> onclick="<?php if(isset($nonepage) && isset($tableclick)) echo $tableclick; elseif((isset($edit) || isset($add)) && $onClick==0){ ?>changer();clickTwo(this);clickOne(this);<?php if(!isset($nonepage)){ ?>setVals2('<?php echo $roomID; ?>','<?php echo $rowcount; ?>');<?php }}?>" style="background: <?php echo $farbe2;?>;cursor:pointer" abbr="<?php echo $farbe2;?>" date="dertest" axis="<?php echo $cellcount+1; ?>" <?php if($overview_options['overview_onmouseover'] == 1){ ?>onmouseover="hoverEffect(this);"<?php } ?>><?php echo $daycount; ?></td><?php
				} else { ?>
        <td id="<?php echo $roomID.'-'.$rowcount.'-'.$preparedCellcount; ?>" title="<?php echo $title; ?>" <?php if(isset($edit) || isset($add) || isset($nonepage)){ ?>onclick="changer();clickTwo(this);clickOne(this);setVals2('<?php echo $roomID; ?>','<?php echo $rowcount; ?>');"<?php } ?> style="background:<?php echo $background2.' '.$colorbgfree;?>" abbr="<?php echo $background2.' '.$colorbgfree;?>" date="dertest" <?php if($overview_options['overview_onmouseover'] == 1){ ?>onmouseover="hoverEffect(this);"<?php } ?> axis="<?php echo $cellcount+1; ?>"><?php echo '0'; ?></td><?php
				}
				continue;
			}
			...
			if(isset($daysOccupied)){
				if(in_array(date($date_pat, $dateToday), $daysOccupied)){
					...
					if($itIS===1){
						?><td id="<?php echo $roomID.'-'.$rowcount.'-'.$preparedCellcount; ?>"<?php echo $addname; ?> title="<?php echo $title_one; ?>" colspan="<?php echo $nights-1-$minusdays; ?>" class="er_overview_cell" date="<?php echo $dateToday-$interval;?>" onclick="<?php echo "location.href = 'admin.php?page=reservations&edit=".$reservationarray[$CountNumberOfAdd]['ID']."';"; ?>" style="background: <?php echo $farbe2;?>;cursor: pointer;text-decoration:none;padding:0px;font: normal 11px Arial, sans-serif;vertical-align:middle;; overflow:hidden;"  abbr="<?php echo $farbe2;?>" title="<?php echo $reservationarray[$CountNumberOfAdd]['name']; ?>" <?php if($overview_options['overview_onmouseover'] == 1){ ?>onmouseover="hoverEffect(this,'<?php echo date(RESERVATIONS_DATE_FORMAT.' H:i', $arrival+$interval).' - '.date(RESERVATIONS_DATE_FORMAT.' H:i', $departure); ?>');"<?php } ?>>
						...
						if(isset($datesHalfOccupied[$cellcount])){
							$value = $datesHalfOccupied[$cellcount]['i'];
							$title .= $datesHalfOccupied[$cellcount]['v'];
							$tableclick = 'document.getElementById(\'easy-table-search-field\').value = \''.$reservationarray[$CountNumberOfAdd]['ID'].','.implode(',', $datesHalfOccupied[$cellcount]['id']).'\';easyreservation_send_table(\'all\', 1);';
							if(isset($id) && in_array($id, $datesHalfOccupied[$cellcount]['id'])) $colorbgfree = '#FCEA74';
          ...
          <td id="<?php echo $roomID.'-'.$rowcount.'-'.$preparedCellcount; ?>"  title="<?php echo $title; ?>" <?php if($borderside == 0) echo 'class="er_overview_cell"'; ?> <?php echo $addname; ?> date="<?php echo $dateToday-$interval;?>" onclick="<?php if(isset($nonepage) && isset($tableclick)) echo $tableclick; elseif((isset($edit) || isset($add) || isset($nonepage)) && $onClick==0){ ?>;changer();clickTwo(this);clickOne(this);setVals2('<?php echo $roomID; ?>','<?php echo $rowcount; ?>');<?php  } elseif($onClick==1){ echo "location.href = 'admin.php?page=reservations&edit=".$reservationarray[$CountNumberOfAdd]['ID']."';"; } ?>" style="background: <?php echo $farbe2;?>; padding:0px; overflow:hidden; text-shadow:none; border-style:none; text-decoration:none; font: normal 11px Arial, sans-serif; vertical-align:middle;<?php if($onClick==1) echo 'cursor:pointer'; ?>" abbr="<?php echo $farbe2;?>" axis="<?php echo $cellcount+1; ?>" <?php if($overview_options['overview_onmouseover'] == 1){ ?>onmouseover="hoverEffect(this);"<?php } ?>>
						<?php echo $value; ?>
          </td><?php
					}
					unset($tableclick);
					...
					if(isset($datesHalfOccupied[$cellcount])){
						...
						$tableclick = 'document.getElementById(\'easy-table-search-field\').value = \''.implode(',', $datesHalfOccupied[$cellcount]['id']).'\';easyreservation_send_table(\'all\', 1);';
						if(isset($id) && in_array($id, $datesHalfOccupied[$cellcount]['id'])) $colorbgfree = '#FCEA74';
					...
					<td id="<?php echo $roomID.'-'.$rowcount.'-'.$preparedCellcount; ?>" title="<?php echo $title; ?>" <?php echo $class; if(isset($edit) || isset($add) || isset($nonepage)){ ?>onclick="<?php if(isset($nonepage) && isset($tableclick)) echo $tableclick; else { ?>changer();clickTwo(this);clickOne(this);setVals2('<?php echo $roomID; ?>','<?php echo $rowcount; ?>');<?php } ?>"<?php } ?> style="background:<?php echo $background2.' '.$colorbgfree;?>" abbr="<?php echo $background2.' '.$colorbgfree;?>" date="<?php echo $dateToday-$interval;?>" <?php if($overview_options['overview_onmouseover'] == 1){ ?>onmouseover="hoverEffect(this);"<?php } ?> axis="<?php echo $cellcount+1; ?>"><?php
					echo $value;
					$wasFull=0;
				}
			} else {
				...
				?><td id="<?php echo $roomID.'-'.$rowcount.'-'.$preparedCellcount; ?>" title="<?php echo $title; ?>" <?php echo $class; if(isset($edit) || isset($add) || isset($nonepage)){ ?>onclick="<?php if(isset($nonepage) && isset($tableclick)) echo $tableclick; else { ?>changer();clickTwo(this);clickOne(this);setVals2('<?php echo $roomID; ?>','<?php echo $rowcount; ?>');<?php } ?>"<?php } ?> style="background:<?php echo $background2.' '.$colorbgfree;?>" abbr="<?php echo $background2.' '.$colorbgfree;?>" date="<?php echo $dateToday-$interval;?>" <?php if($overview_options['overview_onmouseover'] == 1){ ?>onmouseover="hoverEffect(this);"<?php } ?> axis="<?php echo $cellcount+1; ?>"><?php echo $value; ?></td><?php
			}
		}
		unset($daysOccupied,$datesHalfOccupied,$numberOccupied,$reservationarray,$tableclick);
		echo '</tr>';
	}
	$res->destroy();
} ?></tbody>
