...
/* - + - + - + - + - + - + - + - + - + - + - + - + - + - + - + - + - + - + - + - + BULK ACTIONS (trash,delete,undo trash) + - + - + - + - + - + - + - + - + - + - + - + - + - + - + - + - + - + - + - + */
	...
	if(isset($_GET['bulk']) && check_admin_referer( 'easy-main-bulk', 'easy-main-bulk' )){ // GET Bulk Actions
		if(isset($_GET['bulkArr'])) {
			...
			$prompt='<div style="width: 97%; padding: 5px; margin: -11px 0 5px 0;" class="updated below-h2"><p>'.$anzahl.' '.__( 'moved to Trash' , 'easyReservations' ).'. <a href="'.wp_nonce_url('admin.php?page=reservations&bulkArr[]='.$linkundo.'&bulk=2', 'easy-main-bulk').'">'.__( 'Undo' , 'easyReservations' ).'</a></p></div>';
...
function easyRes_sendReq_Overview(x,y,daystoshow) {
	...
	var timpstampanf = '<?php if(isset($timpstampanf)) echo $timpstampanf.'-'.$timpstampend; ?>';
	if(timpstampanf != '') var h = '&timpstampanf=' + timpstampanf;
	else var h = '';
...
if(!isset($approve) && !isset($delete) && !isset($view) && !isset($edit) && !isset($sendmail) && !isset($add)) {
	...
	?><input type="hidden" name="action" value="reservation"><?php
			...
			$items7 = $wpdb->get_var($wpdb->prepare("SELECT COUNT(id) FROM ".$wpdb->prefix ."reservations WHERE  approve='yes' AND NOW() BETWEEN arrivalDate AND DATE_ADD(arrivalDate, INTERVAL nights DAY) + INTERVAL 1 DAY")); // number of total rows in the database
			...
			elseif(isset($typ) AND $typ=="current") { $type="approve='yes'"; $items=$items3; $typlink="&typ=current"; $orders="ASC"; $zeichen ="AND NOW() BETWEEN arrivalDate AND DATE_ADD(arrivalDate, INTERVAL nights DAY) + INTERVAL 1 DAY"; } // If type is pending
			...
			if(isset($specialselector) OR isset($monthselector) OR isset($roomselector)){
				$variableitems = $wpdb->get_var($wpdb->prepare("SELECT COUNT(id) FROM ".$wpdb->prefix ."reservations WHERE $type $monthsql $roomsql $specialsql $zeichen"));
				$items=$variableitems; 
		...
		<table style="width:99%"><tr> <!-- Type Chooser //--> 
			<td style="width:20%;">
				<ul class="subsubsub">
					...
					<li><a href="admin.php?page=reservations&typ=current"><?php printf ( __( 'Current' , 'easyReservations' ));?><span class="count"> (<?php echo $items7; ?>)</span></a> |</li>
					...
					<?php } elseif(isset($typ) AND $typ=="current"){ ?>
					...
					<li><a href="admin.php?page=reservations&typ=current" class="current"><?php printf ( __( 'Current' , 'easyReservations' ));?><span class="count"> (<?php echo $items7; ?>)</span></a> |</li>
					<li><a href="admin.php?page=reservations&typ=pending"><?php printf ( __( 'Pending' , 'easyReservations' ));?><span class="count"> (<?php echo $items3; ?>)</span></a> |</li>
					<li><a href="admin.php?page=reservations&typ=deleted"><?php printf ( __( 'Rejected' , 'easyReservations' ));?><span class="count"> (<?php echo $items2; ?>)</span></a> |</li>
					<li><a href="admin.php?page=reservations&typ=all"><?php printf ( __( 'All' , 'easyReservations' ));?><span class="count"> (<?php echo $items6; ?>)</span></a> |</li>
					<li><a href="admin.php?page=reservations&typ=old"><?php printf ( __( 'Old' , 'easyReservations' ));?><span class="count"> (<?php echo $items4; ?>)</span></a></li>
