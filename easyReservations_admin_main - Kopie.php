...
function reservation_main_page() {
	...
	if($addpricefield){
			...
			$addthecustomp=$presentcustomp.$_POST['pricetitle'].'&:&'.$_POST['priceselected'].':'.$_POST['priceamount'].'&;&';
						...
						foreach( $roomcategories as $roomcategorie ){
							...
							while($roomcounty > $rowcount){
								...
								while($showdatenumber_start < $showdatenumber_end){
									...
									$sql_R2 = "SELECT id, name, nights, arrivalDate FROM ".$wpdb->prefix ."reservations WHERE room='$roomsidentify' AND roomnumber='$rowcount' AND approve='yes' AND roomnumber != '' AND '$datedatumtodaydate' BETWEEN arrivalDate AND DATE_ADD(arrivalDate, INTERVAL nights DAY) LIMIT 0,1";
											...
											<td style="width:64%;" valign="top">
												...
												<table class="widefat" style="width:550px;">
													...
													<tbody>
														...
														foreach($customsmerges as $customprice){
															...
															echo '<td><b><input type="text" name="price_value_'.$thenumber2.'" value="'.$priceexpexplode[0].'" class="regular-text"><input type="text" name="price_amount_'.$thenumber2.'" value="'.$priceexpexplode[1].'" style="width:150px"></b></td></tr>';
														}
														?>
													</tbody>
												...
												<form id="addpricefield" name="addpricefield" method="post" action="admin.php?page=reservations&edit=<?php echo $edit; ?>&addpricefield=<?php echo $edit; ?>"> 
													<table class="widefat">
														...
														<tbody>
															<tr>
																<td nowrap><input type="text" name="pricetitle" style="width:260px" value="Title" onfocus="if (this.value == 'Title') this.value = '';"><br><input type="text" name="priceselected" style="width:260px" value="Selected" onfocus="if (this.value == 'Selected') this.value = '';"><br><input type="text" name="priceamount" style="width:260px" value="Price" onfocus="if (this.value == 'Price') this.value = '';">
																<br><br><p><a href="javascript:{}" onclick="document.getElementById('addpricefield').submit(); return false;" class="button-secondary"><span><?php printf ( __( 'Add custom Field' , 'easyReservations' ));?></span></a></p></td>
															</tr>
														</tbody>
													</table>
												</form>
...
function reservation_add_reservaton() {
			...
			<div id="wrap">
				<?php if($edit){ ?>
				<form method="post" action=""  id="edit"><input type="hidden" name="action" value="edit"/><?php } else {?><form method="post" action=""  id="addreservation"><input type="hidden" name="action" value="addreservation"/><?php } ?>								
