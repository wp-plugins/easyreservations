...
*/
	...
	function easyReservations_add_pages(){  //  Add Pages Admincenter and Order them
		...
		add_menu_page(__('easyReservation','easyReservations'), __('Reservation','easyReservations').' '.$pending, $reservation_main_permission, 'reservations', 'reservation_main_page', RESERVATIONS_IMAGES_DIR.'/logo.png' );
		add_submenu_page('reservations', __('Dashboard','easyReservations'), __('Dashboard','easyReservations'), $reservation_main_permission, 'reservations', 'reservation_main_page');
	...
	*/
	function easyreservations_get_pending(){
		...
		$count = $wpdb->get_var($wpdb->prepare("SELECT COUNT(id) as Num FROM ".$wpdb->prefix ."reservations WHERE approve='' AND arrivalDate > NOW()"));
		return $count;
...
<div style="text-align:center;">[submit Send]</div>';
	...
	*/
	...
	function easyReservations_upgrade(){
		...
		$easyReservations_active_ver=1.4;
		...
		if($easyReservations_installed_ver != $easyReservations_active_ver ){
			...
			if($easyReservations_installed_ver == 1.2 || $easyReservations_installed_ver == 1.3 || $easyReservations_installed_ver == "1.3.1"){
			...
			if($easyReservations_installed_ver == "1.3.2"){
			...
			if($reservtionsTable == 1){
				update_option('reservations_db_version', "1.4");
	...
	function easyReservations_upgrade_notice(){
		echo '<div class="updated">
		   <p>Thanks for updating <b>easyReservations</b> to <b>1.4</b>!<br>View <a href="http://feryaz.de/changelog/">here</a> for a detailed Changelog!<br>
		   Hope you enjoy it!</p>
		</div>';
	}
