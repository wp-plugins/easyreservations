...
function reservation_main_page() {
	...
	if(isset($editthereservation)){
			...
			$wpdb->query($wpdb->prepare("UPDATE ".$wpdb->prefix ."reservations SET arrivalDate='$rightdate', nights='$calcdaysbetween', name='$name', email='$email', notes='$note', room='$room', number='$persons', special='$specialoffer', dat='$dat', roomnumber='$roomex', price='$settepricei', custom='$customfields', customp='$custompfields' WHERE id='$edit' ")) or die (mysql_error());
		...
		if((isset($approve) || isset($delete) || isset($view) || isset($edit)) OR $show_overview_on_list == 1){ ?>			
			<table cellspacing="0" cellpadding="0" class="widefat" style="width:99%;" align="top">
				...
				<?php				
					...
					else { $argss = array( 'type' => 'post', 'category' => $room_category, 'orderby' => 'post_title', 'order' => 'ASC', 'post_status' => 'publish|private', 'numberposts'     => -1 ); }
												...
												<table  class="widefat" style="width:99%;"> <!-- Main Table //-->
													...
													<?php
														...
														<tr class="<?php echo $class; ?> test" height="47px"><!-- Main Table Body //-->
															...
															<td width="17%" class="row-title" valign="top" nowrap><div class="test"><a href="admin.php?page=reservations&view=<?php echo $id;?>"><?php echo $name;?></a><div class="test2" style="margin:5px 0 0px 0;"><a href="<?php echo $pageURL;?>?page=reservations&edit=<?php echo $id;?>"><?php printf ( __( 'Edit' , 'easyReservations' ));?></a> <?php if(isset($typ) AND ($typ=="deleted" OR $typ=="pending")) { ?>| <a style="color:#28a70e;" href="<?php echo $pageURL;?>?page=reservations&approve=<?php echo $id;?>"><?php printf ( __( 'Approve' , 'easyReservations' ));?></a><?php } if(isset($typ) AND ($typ=="" or $typ=="active" or $typ=="pending")) { ?>| <a style="color:#bc0b0b;" href="<?php echo $pageURL;?>?page=reservations&delete=<?php echo $id;?>"><?php printf ( __( 'Reject' , 'easyReservations' ));?></a><?php } if(isset($typ) AND $typ=="trash") { ?>| <a href="<?php echo $pageURL;?>?page=reservations&bulkArr[]=<?php echo $id;?>&bulk=2"><?php printf ( __( 'Restore' , 'easyReservations' ));?></a> | <a style="color:#bc0b0b;" href="<?php echo $pageURL;?>?page=reservations&bulkArr[]=<?php echo $id;?>&bulk=3"><?php printf ( __( 'Delete Permanently' , 'easyReservations' ));?></a><?php } ?> | <a href="<?php echo $pageURL;?>?page=reservations&view=<?php echo $id;?>"><?php printf ( __( 'View' , 'easyReservations' ));?></a></div></div><?php $csv_output .= $name . ", "; ?></td>
